...
def start_map(parser, attrs):
    if not hasattr(parser, 'current_map'):
	init_map_support(parser)
    for name, value in attrs:
	if name == 'name':
	    name = value
    parser.current_map = MapInfo(parser, name)

def end_map(parser):
    print parser.current_map.name
    print parser.current_map.shapes
    parser.image_maps[parser.current_map.name] = parser.current_map
    parser.current_map = None

def init_map_support(parser):
    """Add <MAP> instance variables to parser."""
    parser.image_maps = {}
    parser.current_map = None
